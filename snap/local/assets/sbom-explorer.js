    ...
    async function loadSBOM(arch) {
      ...
      document.getElementById('tab-amd64').setAttribute('aria-pressed', arch === 'amd64' ? 'true' : 'false');
      document.getElementById('tab-arm64').setAttribute('aria-pressed', arch === 'arm64' ? 'true' : 'false');
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch sbom-${arch}.json: ${response.status} ${response.statusText}`);
        }
        ...
        const syftInfo = data.metadata && data.metadata.tools && data.metadata.tools.components
    ...
    function filterSBOM() {
      ...
      const filtered = currentComponents.filter(c => {
        ...
        if (licenseFilter !== 'all') {
          ...
          const hasPermissive = licensesStr.includes('mit') || 
