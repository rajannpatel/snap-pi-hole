            ...
            const SNAP_GIST_DESCRIPTION =
                "snap-pi-hole dashboard data (auto-updated hourly; do not edit)";
            ...
            function statusIconSvg(status) {
                ...
                if (
                    BUILDING_STATES.has(normalized) ||
                    normalized === "building"
                ) {
                    return clockIconSvg;
                }
            ...
            function trendTooltipDescriptor(conclusion, isSuspiciouslyFast) {
                ...
                if (normalized === "cancelled" || normalized === "skipped") {
                    const titlePrefix =
                        normalized === "cancelled" ? "Cancelled " : "Skipped ";
                    return {
            ...
            function updateInstallActivity(rows) {
                ...
                if (!matrixRows.length) {
                    ...
                    const building = matrixRows.filter((row) =>
                        isBuildingStatus(row.status),
                    ).length;
                    const failed = matrixRows.filter((row) =>
                        FAILURE_STATES.has(row.status),
                    ).length;
                    const passed = matrixRows.filter(
            ...
            function findBuildJob(jobs, arch, channel, isGitHub) {
                ...
                if (isGitHub) {
                    ...
                    const prefixes = buildJobNamePrefixes(
                        arch,
                        channel,
                        isGitHub,
                    );
            ...
            function renderDurationTrendChart(trendRows) {
                ...
                let yMin = Math.max(
                    0,
                    Math.floor((minDuration * 0.9) / 10) * 10,
                );
                ...
                const yAt = (value) =>
                    margin.top + ((yMax - value) / yRange) * plotHeight;
                ...
                const maxYLabels = 6;
                const rawStepUnits = Math.max(
                    1,
                    Math.ceil((yMax - yMin) / 10 / (maxYLabels - 1)),
                );
                ...
                points.forEach((point) => {
                    ...
                    dot.setAttribute(
                        "fill",
                        trendPointColor(point.row.conclusion),
                    );
                ...
                const showPoint = (index, pointerX = null) => {
                    ...
                    const conclusion = String(
                        point.row.conclusion || "unknown",
                    ).toLowerCase();
                    const isSuspiciouslyFast =
                        conclusion === "success" &&
                        point.row.duration_seconds < medianDuration * 0.5;
                    ...
                    const descriptor = trendTooltipDescriptor(
                        conclusion,
                        isSuspiciouslyFast,
                    );
                ...
                overlay.addEventListener("mousemove", (event) => {
                    ...
                    const svgPointerX =
                        margin.left + (pointerX / rect.width) * plotWidth;
                    let index = 0;
                ...
                if (durationTrendResizeHandler)
                    window.removeEventListener(
                        "resize",
                        durationTrendResizeHandler,
                    );
                durationTrendResizeHandler = () =>
                    renderDurationTrendChart(trendRows);
            ...
            function renderBuildStatus(buildStatus) {
                ...
                setText(
                    "latest-run-title",
                    latest.number ? `Run #${latest.number}` : "No run data",
                );
                ...
                if (latestLink) {
                    const durationLabel =
                        latest.duration_label || (building ? "0s" : "Unknown");
                    ...
                    const label = latestLink.querySelector(
                        ".workflow-btn__label",
                    );
                    const icon = latestLink.querySelector(".status-chip-logo");
                    const spinner =
                        latestLink.querySelector(".p-icon--spinner");
                ...
                let regressionText =
                    "Not enough data to compute regression baseline yet.";
            ...
            function renderFailedLogs(testMatrix) {
                ...
                if (!links.length) {
                    list.innerHTML =
                        '<li class="p-list__item">No failed distro jobs in latest workflow runs.</li>';
                    return;
                }
                links.forEach((item) => {
            ...
            function renderMatrixRows() {
                ...
                if (!rows.length) {
                    tbody.innerHTML =
                        '<tr><td colspan="5">No matrix rows available.</td></tr>';
                    return;
                }
                rows.forEach((row) => {
                    ...
                    const statusText = (row.status || "unknown").replace(
                        "_",
                        " ",
                    );
                    const statusBadgeAlt = `${row.label} status: ${statusText}`;
                    const logoAlt = row.family
                        ? `${row.family} logo`
        ...
        `;
            ...
            function renderSecurity(security) {
                const archRows = (security.architectures || []).filter(
                    (row) => {
                ...
                const totalVulns = archRows.length
                    ? Math.max(
                          ...archRows.map((row) => row.vulnerabilities ?? 0),
                      )
                    : 0;
                const totalConfined = archRows.length
                    ? Math.max(
                          ...archRows.map(
                              (row) =>
                                  row.confined_mitigation_vulnerabilities ?? 0,
                          ),
                      )
                    : 0;
                ...
                archRows.forEach((row) => {
                    ...
                    const architectureLabel = String(
                        row.architecture || "",
                    ).toUpperCase();
                    ...
                    const reportOnly =
                        row.confined_mitigation_vulnerabilities ?? 0;
                    const rawMatches =
                        row.raw_vulnerability_matches ??
                        actionable + reportOnly;
                    const rawPackages =
                        row.raw_affected_packages ?? actionablePackages;
                    const actionChip =
                        actionable > 0
                    ...
                    const osvUrl = row.report
                        ? `vulnerabilities/${row.report}`
                        : "";
        ...
        `;
            ...
            function renderDependencies() {
                ...
                const trackRun =
                    globalDashboardData?.auto_update?.latest_success_run || {};
                const fallbackUrl =
                    trackRun.url ||
                    `https://github.com/${REPO_SLUG}/actions/workflows/track-upstream-releases.yml`;
                const jobUrl = liveState.trackUpstreamJobUrl || fallbackUrl;
                const syncDurationSeconds =
                    liveState.trackUpstreamJobDurationSeconds ??
                ...
                if (selectedBranch === "stable") {
                    ...
                    if (!rows.length) {
                        body.innerHTML =
                            '<tr><td colspan="5">No dependency rows match current filter.</td></tr>';
                        return;
                    }
                    rows.forEach((item) => {
                        const statusHtml = componentStatusHtml(item);

                        const localCommitShort = item.local_commit
                            ? ` (${item.local_commit.substring(0, 7)})`
                            : "";
                        const upstreamCommit = upstreamCommitLinkHtml(item);
                        const upstreamCommitShort = upstreamCommit
                            ? ` (${upstreamCommit})`
                            : "";
          ...
          `;
                    ...
                    if (!rows.length) {
                        body.innerHTML =
                            '<tr><td colspan="5">No edge dependency rows available.</td></tr>';
                        return;
                    }
                    rows.forEach((item) => {
                        const statusHtml = componentStatusHtml(
                            item,
                            "Up to date",
                        );

                        const localSource = sourceVersionWithCommitHtml(
                            item,
                            "local_tag",
                            "local_commit",
                        );
                        const upstreamSource = sourceVersionWithCommitHtml(
                            item,
                            "upstream_tag",
                            "upstream_commit",
                        );
          ...
          `;
            ...
            function renderReleaseInfo(data) {
                ...
                const stableUpdateAvailable = stableComponents.some(
                    (c) => c.update_available,
                );
                const stableStatusEl = document.getElementById(
                    "stable-track-status",
                );
                if (stableStatusEl) {
                    ...
                    const stableIcon = stableUpdateAvailable
                        ? warningIconSvg
                        : checkIconSvg;
                    const stableText = stableUpdateAvailable
                        ? "Update pending"
                        : "Up to date";
                    const stableShort = stableUpdateAvailable
                        ? "pending"
                        : "current";
                ...
                const edgeUpdateAvailable = edgeComponents.some(
                    (c) => c.update_available,
                );
                const edgeStatusEl =
                    document.getElementById("edge-track-status");
                if (edgeStatusEl) {
                    const edgeClass = edgeUpdateAvailable
                        ? "status-chip status-caution"
                        : "status-chip status-success";
                    const edgeIcon = edgeUpdateAvailable
                        ? warningIconSvg
                        : checkIconSvg;
                    const edgeText = edgeUpdateAvailable
                        ? "Update pending"
                        : "Up to date";
                    const edgeShort = edgeUpdateAvailable
                        ? "pending"
                        : "current";
                ...
                const stableWorkflowEl = document.getElementById(
                    "stable-track-workflow",
                );
                const edgeWorkflowEl = document.getElementById(
                    "edge-track-workflow",
                );
                const workflowHtml = workflowButtonHtml(
                    trackRun.url,
                ...
                if (stableStatusEl) {
                    ...
                    if (containerTable) {
                        setTimeout(() => {
                            const buttons =
                                containerTable.querySelectorAll(
                                    ".workflow-btn",
                                );
                            if (buttons.length === 0) return;
                            let maxWidth = 0;
                ...
                if (!snapState.hasGistData) {
                    ...
                    snapState.generatedAt =
                        data.generated_at || snapState.generatedAt;
                    setSnapClock(
                        (data.snap_package || {}).last_updated ||
            ...
            function snapStatusDescriptor(row) {
                const channel =
                    row.channel || (row.on_stable ? "stable" : "edge");
                ...
                if (
                    typeof freshnessState !== "undefined" &&
            ...
            function renderSnapPackage(snapPackage) {
                ...
                const packageBody =
                    document.getElementById("snap-package-rows");
                if (!packageBody) return;

                const allRows =
                    snapPackage.all_channels || snapPackage.channels || [];
                const selectedRows = [];
                const arches = Array.from(
                    new Set(allRows.map((r) => r.architecture)),
                );

                arches.forEach((arch) => {
                    const archRows = allRows.filter(
                        (r) => r.architecture === arch,
                    );
                    if (selectedBranch === "stable") {
                        const stableEntry = archRows.find(
                            (r) => r.channel === "stable",
                        );
                        if (stableEntry) {
                            selectedRows.push(stableEntry);
                        } else {
                            const edgeEntry = archRows.find(
                                (r) => r.channel === "edge",
                            );
                ...
                let targetVersion = "";
                const currentChannel = rows.find(
                    (c) => c.build_status === "current",
                );
                if (currentChannel) {
                    targetVersion =
                        currentChannel.full_version ||
                    ...
                    rows.forEach((c) => {
                        if (
                            !newest ||
                            new Date(c.released_at) >
                                new Date(newest.released_at)
                        ) {
                            newest = c;
                        }
                    });
                    if (newest) {
                        targetVersion =
                            newest.full_version || newest.version || "";
                    }
                }
                ...
                if (!rows.length) {
                    packageBody.innerHTML =
                        '<tr><td colspan="8">snap package metadata unavailable.</td></tr>';
                    return;
                }
                rows.forEach((row) => {
                    ...
                    const version =
                        row.full_version || row.version || "Unknown";

                    const workflowSnapshot =
                        (row.workflow_runs || {})[selectedBranch] || {};
        ...
        `;
                ...
                setTimeout(() => {
                    const buttons =
                        packageBody.querySelectorAll(".workflow-btn");
                    if (buttons.length === 0) return;
                    let maxWidth = 0;
                    buttons.forEach((btn) => {
                        btn.style.width = "auto";
            ...
            function applyLiveSnapStatus(cicdJobs, cicdRun, lpJobs, lpRun) {
                ...
                rows.forEach((tr) => {
                    ...
                    const runUrl =
                        (job && job.html_url) || (run && run.html_url) || "";

                    if (job) {
                        const status = normalizedLiveStatus(job);
                        if (isBuildingStatus(status)) {
                            isBuilding = true;
                            statusHtml = liveStatusChip(
                                status,
                                isGitHub ? "building" : "publishing",
                                true,
                            );
                        } else if (
                    ...
                    const duration =
                        (job ? liveJobDurationSeconds(job) : null) ??
                        (run ? liveRunDurationSeconds(run) : null);
                ...
                if (cicdJobs && cicdJobs.length) {
                    const stableAmd64Publish = cicdJobs.find((j) => {
                        const name = String(j.name || "").toLowerCase();
                        return (
                            name.startsWith("publish github (stable, amd64)") &&
                            normalizedLiveStatus(j) === "success"
                        );
                    });
                    const edgeAmd64Publish = cicdJobs.find((j) => {
                        const name = String(j.name || "").toLowerCase();
                        return (
                            name.startsWith("publish github (edge, amd64)") &&
                            normalizedLiveStatus(j) === "success"
                        );
                    });
                    ...
                    cicdRun &&
                    cicdRun.status === "completed" &&
                    cicdRun.conclusion === "success"
                ) {
            ...
            async function refreshLiveSnapPackageStatus() {
                ...
                if (
                    liveState.rateRemaining !== null &&
                    liveState.rateRemaining <= LIVE_RATE_FLOOR
                )
                    return;
                const now = Date.now();
                if (
                    liveState.snapOverlayLastFetch &&
                    now - liveState.snapOverlayLastFetch <
                        SNAP_OVERLAY_MIN_REFETCH_MS
                ) {
                    applyLiveSnapStatus(
                        liveState.cicdJobs,
                        liveState.cicdRun,
                ...
                try {
                    ...
                    if (latestCicd) {
                        const latestCicdBuilding = BUILDING_STATES.has(
                            (latestCicd.status || "").toLowerCase(),
                        );
                        const latestChanged =
                            latestCicd.id !== liveState.lastCicdRunId ||
                            !liveState.cicdJobs ||
                            (liveState.cicdRun &&
                                liveState.cicdRun.id !== latestCicd.id);
                    ...
                    if (
                        latestLp &&
                        (latestLp.id !== liveState.lastLpRunId || !lpJobs)
                    ) {
            ...
            function renderReportSummaries(data) {
                ...
                const rawMatches =
                    security.raw_vulnerability_matches ??
                ...
                const confinedMatches =
                    ...
                    Math.max(
                        0,
                        rawMatches - (security.total_vulnerabilities ?? 0),
                    );
                setText(
                    "report-vuln-summary",
            ...
            function renderChannelSwitch(cs) {
                ...
                if (body) {
                    ...
                    if (!rows.length) {
                        body.innerHTML =
                            '<tr><td colspan="6">No channel switch runs recorded.</td></tr>';
                    } else {
                        rows.forEach((row) => {
                            const tr = document.createElement("tr");
                            const detailsTr = document.createElement("tr");
                            const isBuilding = isBuildingStatus(row.status);
                            const statusBadge = liveStatusChip(
                                row.status,
                                "running",
                            );
                            ...
                            const updated = escapeHtml(
                                formatDate(row.updated_at || cs.updated_at),
                            );
                            const details =
                                DashboardChannelSwitch.channelSwitchEvidenceHtml(
                                    row,
                                );
            ...
            function renderChannelSwitchTimeline(cs) {
                const timeline = document.getElementById(
                    "channel-switch-timeline",
                );
                if (!timeline) return;
                timeline.innerHTML =
                    DashboardChannelSwitch.channelSwitchTimelineHtml(cs);
                setText(
                    "channel-switch-summary",
                    cs.summary || "No data available",
                );
            }
            ...
            function setSnapClock(updatedAt, fromGist) {
                ...
                freshnessState.snap.source = fromGist
                    ? "Snap Store · hourly gist"
                    : "build-time snapshot";
            ...
            function fallbackFreshnessNextAt(kind, clock) {
                ...
                if (kind === "live") {
                    ...
                    return (
                        (Number.isNaN(updatedAt) ? Date.now() : updatedAt) +
                        interval
                    );
                }
            ...
            function freshnessDetail(kind) {
                const nextSuffix = (clock) => {
                    const cd = countdownLabel(
                        fallbackFreshnessNextAt(kind, clock),
                    );
                    return cd ? ` · next ${cd}` : "";
                };
                if (kind === "live") {
                    ...
                    if (clock.status === "stale")
                        return `unavailable — using snapshot${nextSuffix(clock)}`;
                    if (!clock.updatedAt)
                        return `connecting…${nextSuffix(clock)}`;
                    return `updated ${relativeTime(clock.updatedAt)}${nextSuffix(clock)}`;
                }
                if (kind === "snap") {
                    const clock = freshnessState.snap;
                    if (!clock.updatedAt)
                        return `${clock.source}${nextSuffix(clock)}`;
                    return `updated ${relativeTime(clock.updatedAt)}${nextSuffix(clock)}`;
                }
                if (kind === "build") {
                    const clock = freshnessState.build;
                    return clock.updatedAt
                        ? `published ${relativeTime(clock.updatedAt)} · on deploy`
                        : "on deploy";
                }
                return "";
            }
            ...
            function freshnessStatus(kind) {
                ...
                if (kind === "snap")
                    return freshnessState.snap.fromGist ? "fresh" : "muted";
                return "muted";
            }
            ...
            function renderFreshnessChips() {
                document
                    ...
                    .forEach((chip) => {
                        ...
                        const detail = chip.querySelector(
            ...
            function tickWorkflowButtons() {
                document
                    .querySelectorAll(".workflow-btn[data-start-time]")
                    .forEach((btn) => {
            ...
            async function fetchRecentRuns() {
                ...
                if (!Number.isNaN(remaining))
                    liveState.rateRemaining = remaining;
            ...
            async function liveMatrixJobsAndRun(cicdRuns) {
                ...
                const newestCompleted = cicdRuns.find(
                    (run) => run.status === "completed",
                );
                if (newestCompleted && newestCompleted !== cicdRuns[0])
                    candidates.push(newestCompleted);
                let latestJobs = null;
                for (const run of candidates) {
                    ...
                    if (
                        jobs &&
                        jobs.some((job) =>
                            (job.name || "").startsWith("distro test ("),
                        )
                    ) {
                        return { jobs, run };
                    }
            ...
            function applyLiveMatrixFromJobs(jobs, run) {
                if (!jobs || !jobs.length)
                    return { matched: false, building: false };
                let matched = 0;
                let building = false;
                ...
                matrixState.rows = matrixState.rows.map((row) => {
                    ...
                    const job =
                        ...
                        jobs.find((candidate) =>
                            (candidate.name || "").startsWith(legacyPrefix),
                        );
                    if (!job) {
                    ...
                    const durationSeconds =
                        liveJobDurationSeconds(job) ??
                        liveRunDurationSeconds(run);
                    const failed = FAILURE_STATES.has(status);
                    const failedUrl = failed
                        ? job.html_url || (run && run.html_url) || ""
                        : "";
                    if (failed) {
            ...
            async function applyLiveChannelSwitch(latestByWorkflow) {
                ...
                const runUpdated =
                    run.updated_at &&
                    run.updated_at !== liveState.lastChannelSwitchRunUpdatedAt;
                ...
                if (shouldFetchJobs) {
                    ...
                    liveState.lastChannelSwitchRunUpdatedAt =
                        run.updated_at || null;
                ...
                const snapshotRow =
                    (snapshot.rows || []).find((row) => row.arch === "arm64") ||
                    {};
                const snapshotMatchesRun =
                    String(snapshot.run_id || "") === String(run.id || "");
                const snapshotHasRevisions = Boolean(
                    snapshot.stable_revision && snapshot.edge_revision,
                );
                const keepSnapshotRevisionEvidence =
                    snapshotMatchesRun || snapshotHasRevisions;
                const snapshotRunData = keepSnapshotRevisionEvidence
                    ? snapshot
                    : {};
                const snapshotRowData = keepSnapshotRevisionEvidence
                    ? snapshotRow
                    : {};
            ...
            function applyLiveBuildStatus(runs) {
                ...
                const trend = cicdRuns.slice(0, 12).map((run) => {
                    ...
                    return {
                        ...
                        duration_seconds:
                            run.status === "completed" ? duration : null,
                ...
                const completed = cicdRuns.filter(
                    (run) => run.status === "completed",
                );
                ...
                if (completed.length && baseline !== null) {
                    const latestCompletedDuration = liveRunDurationSeconds(
                        completed[0],
                    );
                    if (latestCompletedDuration !== null)
                        regression =
                            Math.round(
                                (latestCompletedDuration - baseline) * 10,
                            ) / 10;
                }

                const frequencyDays = liveUpdateFrequencyDays(cicdRuns);
                if (frequencyDays !== null)
                    setText("snap-update-frequency", `${frequencyDays} days`);
                ...
                renderBuildStatus({
                    latest_run: {
                        ...
                        duration_label: humanDuration(
                            liveRunDurationSeconds(latest),
                        ),
                    },
                ...
                const covDuration = humanDuration(
                    liveRunDurationSeconds(latest),
                );
            ...
            async function applyLiveTrackUpstream(latestByWorkflow) {
                ...
                if (containerTable) {
                    setTimeout(() => {
                        const buttons =
                            containerTable.querySelectorAll(".workflow-btn");
                        if (buttons.length === 0) return;
                        let maxWidth = 0;
                        buttons.forEach((btn) => {
                            btn.style.width = "auto";
            ...
            async function refreshLiveData() {
                ...
                const lpRuns = runs.filter(
                    (run) =>
                        (run.path || "").endsWith("/launchpad-builds.yml") &&
                        run.head_branch === branchName,
                );
                ...
                if (latestCicd) {
                    const effective =
                        latestCicd.status === "completed"
                            ? latestCicd.conclusion || ""
                            : latestCicd.status || "";
                    const cicdBuilding = BUILDING_STATES.has(
                ...
                if (latestLp) {
                    lpRun = latestLp;
                    const lpBuilding = BUILDING_STATES.has(
                        (latestLp.status || "").toLowerCase(),
                    );
                    if (
                        lpBuilding ||
                ...
                try {
                    channelSwitchBuilding =
                        await applyLiveChannelSwitch(latestByWorkflow);
                } catch (error) {
                    channelSwitchBuilding = false;
                }

                const lpBuildingStatus =
                    latestLp &&
                    BUILDING_STATES.has((latestLp.status || "").toLowerCase());
                ...
                const anyRunning = runs.some((run) =>
                    BUILDING_STATES.has((run.status || "").toLowerCase()),
                );
                freshnessState.live.status = building
                    ? "building"
                    : anyRunning
                      ? "active"
                      : "idle";
            ...
            async function refreshReleaseLag() {
                ...
                for (const item of dependencyRows) {
                    ...
                    if (release.html_url)
                        item.release_notes_url = release.html_url;
                        ...
                        if (commitResponse.ok) {
                            ...
                            if (commitData && commitData.sha) {
                                ...
                                item.update_available = Boolean(
                                    ...
                                    item.local_commit !== item.upstream_commit,
                                );
                            }
                        }
                    } catch (e) {
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            if (
                                ...
                                item.update_available = Boolean(
                                    item.local_commit &&
                                    item.local_commit !== commitData.sha,
                                );
                                changed = true;
                            }
                        }
            ...
            // catches up.
            function shouldApplySnapPayload(
                currentGeneratedAt,
                incomingPayload,
            ) {
                const incomingGeneratedAt =
                    incomingPayload && incomingPayload.generated_at;
            ...
            function applySnapData(payload) {
                ...
                snapState.generatedAt =
                    payload.generated_at || snapState.generatedAt;
                setSnapClock(
                    payload.data_last_updated ||
                        (payload.snap_package || {}).last_updated,
                    true,
                );
                renderFreshnessChips();
                return true;
            ...
            async function maybeRefreshSnap() {
                ...
                if (
                    snapState.lastFetch &&
                    now - snapState.lastFetch < SNAP_MIN_REFETCH_MS
                )
                    return;
                ...
                if (payload) {
                    ...
                    if (!applied) {
                        snapState.nextAt = Math.min(
                            Date.now() + SNAP_MIN_REFETCH_MS,
                            boundary,
                        );
            ...
            async function loadDashboardData() {
                ...
                if (!response.ok) {
                    throw new Error(
                        `Failed to load dashboard-data.json (${response.status})`,
                    );
                }
            ...
            function renderBranchData() {
                ...
                const buildStatus =
                    globalDashboardData.build_status?.[selectedBranch] || {};
                const testMatrix =
                    globalDashboardData.test_matrix?.[selectedBranch] || {};

                const snapPackagesTitle = document.getElementById(
                    "snap-packages-title",
                );
                if (snapPackagesTitle) {
                    snapPackagesTitle.textContent =
                        selectedBranch === "stable"
                            ? "Stable channel snap packages"
                            : "Edge channel snap packages";
                }
            ...
            function renderBakedSections(data, options = {}) {
                const renderChannelSwitchSection =
                    options.renderChannelSwitchSection !== false;
                freshnessState.build.updatedAt =
                    data.generated_at || data.data_last_updated || null;
            ...
            async function initDashboard() {
                ...
                try {
                    ...
                    renderFreshnessChips();
                    const matrixBody = document.getElementById(
                        "compatibility-matrix-body",
                    );
                    if (matrixBody) {
