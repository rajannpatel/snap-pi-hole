...
if ! command -v copilot >/dev/null 2>&1; then
  printf 'model-discovery: warning: copilot CLI not found\n' >&2
  exit 0
fi

if ! command -v timeout >/dev/null 2>&1; then
  printf 'model-discovery: warning: timeout command not found; skipping copilot discovery\n' >&2
  exit 0
fi
...
help_text=$(run_help_with_timeout "copilot --help" copilot --help) || exit 0
list_cmd=()

if grep -Eq '(^|[[:space:]])models([[:space:],]|$)' <<<"$help_text"; then
  list_cmd=(copilot models)
elif grep -Eq '(^|[[:space:]])list-models([[:space:],]|$)' <<<"$help_text"; then
  list_cmd=(copilot list-models)
elif grep -Eq '(^|[[:space:]])model([[:space:],]|$)' <<<"$help_text"; then
  model_help=$(run_help_with_timeout "copilot model --help" copilot model --help) || exit 0
  if grep -Eq '(^|[[:space:]])list([[:space:],]|$)' <<<"$model_help"; then
    list_cmd=(copilot model list)
  fi
fi

if [ "${#list_cmd[@]}" -eq 0 ]; then
  printf 'model-discovery: warning: copilot CLI has no recognized model-list subcommand\n' >&2
  exit 0
fi

output=$(run_with_timeout "copilot model-list command" "${list_cmd[@]}") || {
  printf 'model-discovery: warning: copilot model-list command failed\n' >&2
  exit 0
}
